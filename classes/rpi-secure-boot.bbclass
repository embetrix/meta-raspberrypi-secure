...
#
#   If no signing key is set and key generation is enabled
#   a temporary key can be generated by enabling:
#   RPI_SECURE_BOOT_SIGN_KEY_GEN = "1"
...
IMAGE_CMD:rpi-secboot () {
    ...
    if [ "${RPI_SECURE_BOOT_SIGN}" = "1" ]; then
        ...
        if [ -z "${SIGN_KEY}" ] && [ "${RPI_SECURE_BOOT_SIGN_KEY_GEN}" = "1" ]; then
            SIGN_KEY="${RPI_SECURE_BOOT_SIGN_KEY_GENERATED}"
            [ -f "${SIGN_KEY}" ] || \
                openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:2048 -out "${SIGN_KEY}" \
                || bbfatal "Failed to generate signing key ${SIGN_KEY}"
            chmod 0600 "${SIGN_KEY}"
        fi
        ...
        if [ -f "${SIGN_KEY}" ]; then
            ...
            bbwarn "No secure boot signing key configured or generated; skipping signing"
        fi
    fi
}
